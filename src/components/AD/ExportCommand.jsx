...
export default function ExportCommand({ sendCommand }) {
    ...
    const exportXlsx = (event) =>
        exportInsight(event, "xlsx", cellMergedRef.current.checked, includeFiltersRef.current.checked);

    return (
        <Card className={classes.card} variant="outlined">
            <CardContent className={classes.cardContent}>
                <FormControlLabel
                    control={<Checkbox inputRef={cellMergedRef} name="cellMerged" color="primary" />}
                    label="Keep attribute cells merged"
                />
                <FormControlLabel
                    control={<Checkbox inputRef={includeFiltersRef} name="includeFilters" color="primary" />}
